          ...
          children: [
            ...
            FutureBuilder<Map<String, dynamic>>(
              ...
              builder: (context, snapshot) {
                ...
                final examinationData = snapshot.data!;
                final examInfo =
                    examinationData['PHIEUKHAM'] as Map<String, dynamic>?;

                // Kiểm tra tính hợp lệ của phiếu khám
                final bool isDoctorActive =
                    examInfo?['BACSI']?['TrangThai'] ?? false;
                final bool isSpecialtyActive =
                    examInfo?['CHUYENKHOA']?['TrangThaiHD'] ?? false;
                final bool isPackageActive =
                    examInfo?['price_packages']?['is_active'] ?? false;

                // Một phiếu khám hợp lệ khi bác sĩ, chuyên khoa và gói dịch vụ (nếu có) đều còn hoạt động
                final bool isValid = isDoctorActive &&
                    isSpecialtyActive &&
                    (examInfo?['price_packages'] == null || isPackageActive);
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        _buildInfoRow(
                            'Mã phiếu khám',
                            examInfo?['MaPK']?.toString().substring(0, 6) ??
                                'Không xác định'),
                        _buildInfoRow(
                            'Chuyên khoa',
                            examInfo?['CHUYENKHOA']?['TenCK'] ??
                                'Không xác định'),
                        if (!isValid) ...[
                          ...
                          Container(
                            ...
                            child: Column(
                              ...
                              children: [
                                if (!isDoctorActive)
                                  _buildInvalidStatusRow(
                                      'Bác sĩ không còn hoạt động'),
                                if (!isSpecialtyActive)
                                  _buildInvalidStatusRow(
                                      'Chuyên khoa đã ngừng hoạt động'),
                                if (examInfo?['price_packages'] != null &&
                                    !isPackageActive)
                                  _buildInvalidStatusRow(
                                      'Gói dịch vụ đã ngừng hoạt động'),
                              ],
                            ),
                          ),
                        ],
